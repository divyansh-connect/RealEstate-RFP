      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-6 border-b border-[#202641]">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <span className="text-xs font-mono font-bold px-2.5 py-0.5 rounded bg-[#7c5cfc]/15 text-[#9b8afb] border border-[#7c5cfc]/30 uppercase">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="px-4 py-2 bg-[#12162a] border border-[#202641] hover:border-[#7c5cfc]/40 text-[#f5f5f7] text-xs font-semibold rounded-xl transition-all flex items-center gap-2 cursor-pointer"
          >
        ...
        <div className="executive-panel rounded-2xl p-6 lg:col-span-2">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              ...
              <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-[#7c5cfc] text-white">
                {needsHumanCount + leadsWithAddressCount} Items
              </span>
            </div>
            {!isReadOnly && (
              <button onClick={() => onNavigate('leadqueue')} className="text-xs text-[#9b8afb] hover:underline">
          ...
          <div className="space-y-3">
            {conversations.slice(0, 3).map((conv) => (
              <div
                key={conv.id}
                onClick={() => onNavigate('conversations')}
                className="p-3.5 rounded-xl bg-[#070811] border border-[#202641] hover:border-[#7c5cfc]/40 transition-all cursor-pointer flex items-center justify-between group"
              >
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 rounded-lg bg-[#101323] text-[#f5f5f7] font-bold text-xs flex items-center justify-center border border-[#202641]">
                    {conv.grade}
                  </div>
                  <div>
                    <div className="text-xs font-bold text-white flex items-center gap-2">
                      {conv.realtorName} &bull; <span className="text-[#a7adc0] font-normal">{conv.brokerage}</span>
                    </div>
                    <div className="text-[11px] text-[#a7adc0] line-clamp-1 mt-0.5">
                      {conv.latestMessage}
                    </div>
                  </div>
                </div>
